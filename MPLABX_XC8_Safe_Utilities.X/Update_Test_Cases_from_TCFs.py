...
# List of all the various modes
  ...
  if not os.path.exists(tcf_folder + '\\' + tcf_name):
    log.write ('parse_tcf: Error: tcf file : ' + tcf_folder + '\\' + tcf_name + ' does not exist\n')
  else: 
    log.write ('parse_tcf: Parsing tcf file : ' + tcf_folder + '\\' + tcf_name + '\n')
  
    # Iterate over every line in the tcf file
    with open(tcf_folder + '\\' + tcf_name, 'r') as tcf:
      for line in tcf:
        if ( mode == Search ):
